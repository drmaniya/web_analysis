<?php 
	...
	class Reports extends CI_Controller
	{
			...
			function get_reports($website_id=0)
			{
...
//print_r($insights->getRaw());
			...
			if(@$web_status['web_status'] == 1)
			{
				...
				@$this->session->set_userdata('website_iddd',$website_id);
				...
				//echo "dasa";
				//$temp=4;
				//@$data['tracking_data']=$this->Paggination_model->pagination_data(3,4,"website_tracking","user/Reports/get_reports/",$website_id,'website_id','',$temp);
				//echo "das";
				//die;
			...
			@$data['cnt_ymbrowser']=$this->Reports_model->get_ybrowser($website_id,"Microsoft");
				...
				//$insights = new SSX\API\Google\PageSpeed\ApiClient("http://google.com",			"AIzaSyCL5qxwBRMrzBhAvTnwRcXL_2F8NtchHRA");
					...
					//echo $noof;
						...
						$this->load->view('user/Reports_display',$data);	
					}
					else
					{
						@$this->load->view('user/Warning_report',$data);
					}
			}
			...
			function ajax_search_webtrack()
			{
				//$src_fed = $this->input->post('searchtxt');	
				//$rec = "select * from website_tracking where user_ip like '$src_fed%' ";
				//$temp = 2;
				
				//$arr['user_data'] = $this->Paggination_model->pagination_data(3,4,$rec,"admin/Reports/ajax_search_webtrack/",0,0,0,$temp);
				
				
				@$arr['tracking_data'] = $this->Reports_model->search_webtrackdata();
				//echo "return ";
				//die;
				$this->load->view('user/Search_websitetrack',$arr);	
			}
